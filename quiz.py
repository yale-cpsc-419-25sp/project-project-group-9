...
def generate_user_id(name):
    ...
    return f"{first_three}{random_digits}"
...
def quiz_form():
    ...
    affinity_groups_list = ["International","FGLI","BIPOC","LGBTQ+","WGI in STEM","PWD"]
    interests_list = ["Nonprofit & Community Organizing/Engagement","Finance","Consulting",
        "Public Health & Healthcare","Computer Science","Entrepreneurship","Education & Academic",
        "Law & Public Policy","Creative Arts & Design","Entertainment","Media & Communications",
        "Sports","Journalism","Research","Sales & Marketing"
    ]
    seeking_list = ["Finding a Job","Finding Community","Extracurriculars","Networking","Research","Graduate School","Coursework","Surviving at Yale"]
...
def submit_quiz():
    # Get the current CAS username if available (or any other identifier)
    cas_username = session.get('cas_username', None)
    ...
    majors           = set(request.form.getlist("majors[]"))
    affinity_groups  = set(request.form.getlist("affinity_groups[]"))
    interests        = set(request.form.getlist("interests[]"))
    seeking          = set(request.form.getlist("mentorship_seeking[]"))
    offering         = set(request.form.getlist("mentorship_offering[]"))
    ...
    if not roles: errors.append("Please choose at least one role.")
    ...
    # Insert into Users table, cas_username will link multiple profiles to same user
    cur.execute("""
        INSERT INTO Users (
            name, pronoun, residential_college, college_year,
            headshot_path, extracurriculars, work_experience, bio, 
            hashed_password, cas_username
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        name, pronoun, residential_college, college_year,
    ...
    # Get the newly created user_id (which is an integer due to AUTOINCREMENT)
    user_id = cur.lastrowid
    
    # Store this user_id in the session
    session['user_id'] = user_id
    
    # Use this user_id for all relationships
    
    # Majors
    for m in majors:
        mid = get_or_create_id(conn, "Majors", "name", m)
        cur.execute("INSERT OR IGNORE INTO User_Majors (user_id, major_id) VALUES (?, ?)", 
            (user_id, mid))

    # Interests
    for i in interests:
        iid = get_or_create_id(conn, "Interests", "name", i)
        cur.execute("INSERT OR IGNORE INTO User_Interests (user_id, interest_id) VALUES (?, ?)", 
            (user_id, iid))

    # Affinity Groups
    for a in affinity_groups:
        aid = get_or_create_id(conn, "Affinity_Groups", "name", a)
        cur.execute("INSERT OR IGNORE INTO User_Affinity_Groups (user_id, group_id) VALUES (?, ?)", 
            (user_id, aid))

    # Mentorship Seeking
    for s in seeking:
        sid = get_or_create_id(conn, "Mentorship_Topics", "name", s)
        cur.execute("INSERT OR IGNORE INTO User_Seeking_Mentorship (user_id, topic_id) VALUES (?, ?)", 
            (user_id, sid))

    # Mentorship Offering
    for o in offering:
        oid = get_or_create_id(conn, "Mentorship_Topics", "name", o)
        cur.execute("INSERT OR IGNORE INTO User_Offering_Mentorship (user_id, topic_id) VALUES (?, ?)", 
            (user_id, oid))

    # Roles (like admin, student, etc.)
    for r in roles:
    ...
    # Return the user_id for the redirect
    return user_id